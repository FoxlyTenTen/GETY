...
/* ─────────────────────────────────────────────────────────────────────────────
   HOW DATA IS FETCHED:
   - This page receives `scanId` from the URL params (set by scanhistorylist.tsx)
   - It looks up the scan in the local ScanContext history by id.
   - TODO: Replace with Supabase query when you're ready to connect the database.
───────────────────────────────────────────────────────────────────────────── */
...
const COLORS = {
    ...
    textMuted: '#6b7280',
    expertCard: '#e6edea',
    progressCurrentBg: '#a8e6cf',
    progressCurrentText: '#1e5b43',
    grayStepBg: '#e5e7eb',
    grayStepIcon: '#4b5563',
    warning: '#fcece3',
    warningText: '#a4715c',
...
export default function DetailHistoryPage() {
    ...
    // ── Step 1: Get the scanId from navigation params ──────────────────────────
    const { scanId } = useLocalSearchParams<{ scanId: string }>();
    const { history } = useScan();

    // ── Step 2: Look up the scan in local history ──────────────────────────────
        ...
        return (
            <SafeAreaView style={styles.container}>
                <View style={styles.center}>
                    ...
                    <Text style={styles.loadingSubText}>scanId: {scanId ?? 'none'}</Text>
    ...
    // ── Step 3: Derive display values from local ScanContext record ────────────
    const steps    = (scan.treatmentSteps ?? []);
    const diseaseName   = scan.diseaseName ?? 'Unknown Disease';
    const description   = scan.description ?? '';
    const fungicide     = scan.fungicide ?? 'N/A';
    const waterMix      = scan.waterMix ?? 'N/A';
    const dayPlan       = scan.dayPlan ?? 14;
    const followUpDays  = scan.followUpDays ?? 14;
    const whatToDo      = scan.whatToDo ?? [];
    const location      = scan.scanAddress || scan.location || 'Unknown Plot';
    ...
    const currentStepIndex = steps.findIndex(s => s.status === 'current') + 1;
        ...
        <SafeAreaView style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.imageCard}>
                    <Image
                        source={scan.imageUri ? { uri: scan.imageUri } : require('@/assets/images/leaf.jpeg')}
                        style={styles.sampleImage}
                        resizeMode="cover"
                    />
                    <View style={[styles.riskBadgeAbs, { backgroundColor: riskBg }]}>
                ...
                <View style={styles.diagnosisCard}>
                    ...
                    <View style={styles.progressBarBg}>
                        <View style={[styles.progressBarFill, { width: `${scan.confidence}%` as any }]} />
                    </View>
                    <Text style={styles.diagnosisDesc}>{description}</Text>
                </View>
                ...
                <View style={styles.expertCard}>
                    ...
                    <Text style={styles.expertDesc}>
                        Apply <Text style={{ fontWeight: '700' }}>{fungicide}</Text> fungicide immediately.
                    ...
                    <View style={styles.expertTagsRow}>
                        <View style={styles.expertTag}>
                            ...
                            <Text style={styles.expertTagText}>{dayPlan} Day Plan</Text>
                        </View>
                        <View style={styles.expertTag}>
                            <MaterialCommunityIcons name="water-percent" size={14} color={COLORS.primary} />
                            <Text style={styles.expertTagText}>{waterMix}</Text>
                        </View>
                        <View style={styles.expertTag}>
                            <Ionicons name="refresh-outline" size={14} color={COLORS.primary} />
                            <Text style={styles.expertTagText}>Follow up: {followUpDays}d</Text>
                        </View>
                    </View>
                </View>

                {/* ── GPS Location ── */}
                {scan.scanLat && scan.scanLng && (
                    <View style={styles.gpsBlock}>
                        ...
                        <View style={styles.gpsCoordRow}>
                            <View style={styles.gpsCoordItem}>
                                ...
                                <Text style={styles.gpsCoordValue}>{scan.scanLat.toFixed(6)}</Text>
                            </View>
                            <View style={styles.gpsCoordDivider} />
                            <View style={styles.gpsCoordItem}>
                                <Text style={styles.gpsCoordLabel}>LONGITUDE</Text>
                                <Text style={styles.gpsCoordValue}>{scan.scanLng.toFixed(6)}</Text>
                            </View>
                        </View>
                        {scan.scanAddress && (
                            <View style={styles.gpsAddressRow}>
                                <Ionicons name="map-outline" size={13} color={COLORS.textMuted} />
                                <Text style={styles.gpsAddressText}>{scan.scanAddress}</Text>
                            </View>
                        )}
                    </View>
                )}
                    ...
                    <>
                        ...
                        <View style={styles.timelineContainer}>
                            ...
                            {steps.map(step => {
                                ...
                                if (step.status === 'completed') return (
                                    <View key={step.id} style={styles.timelineStep}>
                                        ...
                                        <View style={[styles.stepContent, { paddingTop: 2 }]}>
                                            ...
                                            {dueLabel && <Text style={styles.stepDue}>✅ {dueLabel}</Text>}
                                        </View>
                                    </View>
                                );
                                ...
                                if (step.status === 'current') return (
                                    <View key={step.id} style={styles.timelineStep}>
                                        ...
                                        <View style={[styles.stepContent, styles.activeStepCard]}>
                                            ...
                                            <Text style={styles.stepDesc}>{step.desc}</Text>
                                            {dueLabel && <Text style={styles.stepDue}>📅 Due: {dueLabel}</Text>}
                                        </View>
                                    </View>
                                );
                                ...
                                return (
                                    <View key={step.id} style={styles.timelineStep}>
                                        <View style={[styles.stepIcon, { backgroundColor: COLORS.grayStepBg }]}>
                                            <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: COLORS.grayStepIcon }} />
                                        </View>
                                        <View style={[styles.stepContent, { paddingTop: 2 }]}>
                                            <Text style={styles.stepTitle}>{step.title}</Text>
                                            <Text style={styles.stepDesc}>{step.desc}</Text>
            ...
            <View style={styles.bottomActions}>
                <TouchableOpacity
                    ...
                    onPress={() => router.push({ pathname: '/pages/milestone_detail' as any, params: { scanId: scan.id } })}
                    activeOpacity={0.85}
                >
...
const styles = StyleSheet.create({
    ...
    loadingSubText: { fontSize: 11, color: '#d1d5db' },
    backLink: { marginTop: 8 },
    backLinkText: { color: COLORS.primary, fontWeight: '700', fontSize: 15 },

    header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingVertical: 12 },
    backButton: { width: 36, height: 36, borderRadius: 18, backgroundColor: '#e8f5e9', alignItems: 'center', justifyContent: 'center' },
    ...
    imageCard: { width: '100%', height: 240, borderRadius: 24, overflow: 'hidden', marginBottom: 16, backgroundColor: '#011222' },
    sampleImage: { width: '100%', height: '100%', opacity: 0.9 },
    riskBadgeAbs: { position: 'absolute', top: 14, right: 14, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 12 },
    ...
    capturedTag: {
        ...
        backgroundColor: 'rgba(255,255,255,0.88)', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20, gap: 6,
    },
    ...
    diseaseIconContainer: { width: 44, height: 44, borderRadius: 22, backgroundColor: '#ffebee', alignItems: 'center', justifyContent: 'center' },
    diagnosisLabel: { fontSize: 11, fontWeight: '700', color: COLORS.textMuted, letterSpacing: 1, textTransform: 'uppercase', marginBottom: 2 },
    ...
    stepNumberCircle: { width: 24, height: 24, borderRadius: 12, backgroundColor: '#e8f5e9', alignItems: 'center', justifyContent: 'center' },
    ...
    expertTag: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20, gap: 6 },
    ...
    gpsAddressRow: { flexDirection: 'row', alignItems: 'center', gap: 6, borderTopWidth: 1, borderColor: '#c6f6d5', paddingTop: 10 },
    gpsAddressText: { fontSize: 12, color: COLORS.textMuted, flex: 1 },
    ...
    stepIcon: { width: 24, height: 24, borderRadius: 12, alignItems: 'center', justifyContent: 'center', marginRight: 16, zIndex: 1 },
    stepContent: { flex: 1 },
    activeStepCard: { backgroundColor: '#fff', borderRadius: 16, padding: 16, marginTop: -12, borderColor: '#eee', borderWidth: 1 },
    ...
    currentBadge: { backgroundColor: COLORS.progressCurrentBg, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 12, marginLeft: 8 },
    currentBadgeText: { fontSize: 9, fontWeight: '800', color: COLORS.progressCurrentText },

    bottomActions: { paddingHorizontal: 20, paddingTop: 16, paddingBottom: 24, backgroundColor: COLORS.background },
    primaryBtn: {
